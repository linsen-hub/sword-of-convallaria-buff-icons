...
3. **技能图标**：
   ...
   - 跑 `python normalize_skill_icon.py <角色名>`：脚本会检测每张图的圆形范围、缩放到 120px 圆直径、贴进 128x128 透明画布。
   - 抠外部背景有两种模式，脚本按 `处理脚本或素材/圆形通道贴图.png` 是否存在自动选，启动时会打印用的是哪种：
     - **通道贴图模式（推荐）**：外部一律按这张圆形通道贴图裁切，圆内像素 100% 原样保留。缩放不认死 120px，而是量出贴图里圆的实际大小，把每张原图的圆**自动拉到撑满这个圆**——原图圆大小不一（实测同批能差 100px）也能对齐，不用手工给缩放比例。贴图可以是 128x128 整张画布，也可以是别的尺寸（自动缩到 128）；带 alpha 通道的用 alpha，纯 RGB/灰度图用灰度（白=保留、黑=裁掉）。
     - **色差抠图模式（回退）**：贴图不在时按"和背景色的色差"抠，圆内接近背景色的白色高光会被误擦，只当兜底用。
...
| 现象 | 处理 |
|---|---|
| 成品圆周有一圈白边（通道贴图模式下） | 原图圆的外轮廓是羽化的，边界上有 1~3px 半白半彩的过渡像素；只要"刚好撑满"通道圆，这圈过渡就正好压在圆最外圈。脚本用 `OVERSCAN_PX`（默认 1.0）多放大一点，把羽化带推到通道圆外裁掉。还有白边就把这个值调到 1.5~2.0；但别调过头，超过 2.5 会把画面内部的亮光效拉到边缘，边缘反而更亮。 |
| 圆内的白色高光被擦掉（圆环中心、星芒发白处变透明） | 说明在跑色差抠图模式：它按"和四角背景色的色差"判断背景，圆内近白的高光会被一起判成背景打穿。放一张 120x120 圆形 alpha 通道贴图到 `技能图标/处理脚本或素材/圆形通道贴图.png`，重跑即切换到通道贴图模式，圆内不再做任何按颜色的抠除。 |
