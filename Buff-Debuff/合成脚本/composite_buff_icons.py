...
8. 原图不要求必须是透明底：如果给的是纯色背景（黑底白图标 / 白底深色图标，没
   ...
   前用 PS 处理好透明通道再给我。
9. 角色/技能名字本身（CHARACTERS 映射）来自具体项目的需求表，可能含未发布
   内容，不写在这个脚本里——从同目录的 characters.json 读取，这个文件不进
   版本控制（见 .gitignore），每个项目自己维护；characters.example.json 是
   进版本控制的占位示例，展示怎么写。
"""
...
CONFIG_PATH = os.path.join(SCRIPT_DIR, "characters.json")
CONFIG_EXAMPLE_PATH = os.path.join(SCRIPT_DIR, "characters.example.json")
...
def load_characters():
    """从 characters.json 读角色映射；不存在就用示例文件并提醒。

    每个角色一个字典：符号图文件名 -> [[底图文件名(不含.png), 箭头角标文件名或None, 输出文件名], ...]
    底图文件名可以是纯色（green/red/purple/blue/yellow）也可以是金框款
    （goldframe_green/goldframe_red/goldframe_yellow/goldframe_blue）。
    """
    path = CONFIG_PATH if os.path.exists(CONFIG_PATH) else CONFIG_EXAMPLE_PATH
    if path == CONFIG_EXAMPLE_PATH:
        print(f"[提示] 未找到 {CONFIG_PATH}，先用示例配置 {CONFIG_EXAMPLE_PATH} 跑一遍；"
              f"正式使用请复制一份改成 characters.json（该文件不进 git）。")
