...
2. 打开（或新建）`合成脚本/characters.json`——没有就复制一份 `characters.example.json` 改名。在里面加一段：
   ```json
   "角色名": {
     "符号文件名.png": [
       ["底图名(不含.png)", "箭头文件名或null", "输出文件名.png"]
     ]
   }
   ```
   底图名：纯色 `green`/`red`/`purple`/`blue`/`yellow`，或金框款 `goldframe_green`/`goldframe_red`/`goldframe_yellow`/`goldframe_blue`。箭头文件名对应 `底图模板/` 里的 `{color}_arrow.png`（单）/`{color}_arrow2.png`（双），没有就填 `null`。
3. 颜色怎么选：看需求表备注——一般"绿底"=buff、"红底"=debuff，"金框"代表特殊/强化态；备注写了"参考【某图标】"就照抄那张的底色；备注含糊或自相矛盾时问用户，不要猜。**表格截图看不清楚宁可放大/多截一次也别凑合读**——之前就因为看错"红底"/"绿底"两个字导致一枚图标底色配错，这类字段必须逐字核对原文，不能凭印象。
4. 运行脚本：`python composite_buff_icons.py`（会处理 `characters.json` 里所有角色；没有这个文件会用示例配置跑并提示），产物在 `合成成品/<角色名>/`，附一张 `_预览联系表.png`。
5. 用 Read 工具看预览联系表 + 几张代表性单图，再对照需求表逐条核对底色/金框是否用对，检查有没有压边框、居中是否合理、线条是否清晰。
